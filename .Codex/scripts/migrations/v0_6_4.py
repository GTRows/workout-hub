"""Migration v0.6.4: rewrite ``## Communication`` in AGENTS.md so help and
...
That bullet tells Codex to keep tables in English even when the user has
        ...
        actions.append(
            Action(
                ...
                note=(
                    ...
                    "be parsed; not rewritten. Edit AGENTS.md manually."
                ),
            )
        )
        return actions
...
def _apply_one(root: Path, a: Action) -> None:
    ...
    if a.kind == "note":
        log = root / ".Codex" / "migration-log.md"

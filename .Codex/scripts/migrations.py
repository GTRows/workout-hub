#!/usr/bin/env python3
"""
Versioned migration runner for the Codex port of claude-code-template.

Discovers per-version migration modules under
``.Codex/scripts/migrations/`` and applies any whose target version is
strictly greater than the project's current ``.Codex/VERSION`` and less
than or equal to the upstream target version supplied on the command line
(or read from a fresh ``.Codex/VERSION`` if no flag is given).
...
def run(root: Path, target_version: str, dry_run: bool, log: Callable[[str], None]) -> int:
    ...
    for ver, path in plan_set:
        ...
        if not dry_run:
            ...
            (root / ".Codex" / "VERSION").write_text(ver_str + "\n", encoding="utf-8")
            log(f"  v{ver_str}: applied; bumped .Codex/VERSION")
    return 0
...
def main() -> int:
    ...
    parser.add_argument(
        "--target",
        help="target version to migrate to. Default: read .Codex/VERSION (so this is a no-op unless used by /gtr:update with a fresher copy).",
    )
    parser.add_argument(
        "--check",
        action="store_true",

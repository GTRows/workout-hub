#!/usr/bin/env python3
"""
Generate .Codex/.template-manifest.json by hashing every template-owned file
that currently exists in the project. Used by /gtr:setup at install time and
by /gtr:update after applying upstream changes.

Usage:
    python3 .Codex/scripts/manifest.py [--write] [--check]

Without flags, prints the computed manifest to stdout (JSON).
--write   overwrite .Codex/.template-manifest.json with the computed manifest.
--check   exit 1 if any template-owned file's sha differs from the manifest;
          used by /gtr:doctor and /gtr:update to detect user modifications.
"""
...
TEMPLATE_PATHS = [
    ".Codex/commands/*.md",
    ".Codex/commands/gtr/*.md",
    ".Codex/hooks/*.py",
    ".Codex/hooks/optional/*.py",
    ".Codex/scripts/*.py",
    ".Codex/scripts/migrations/*.py",
    ".Codex/docs/*.md",
    ".Codex/agents/*.toml",
    ".Codex/TIPS.md",
    ".Codex/VERSION",
...
def load_existing(root: Path) -> dict | None:
    mfile = root / ".Codex" / ".template-manifest.json"
...
def write_manifest(root: Path, manifest: dict) -> Path:
    ...
    /gtr:setup and /gtr:update sessions from leaving a corrupted manifest.
    """
    mfile = root / ".Codex" / ".template-manifest.json"

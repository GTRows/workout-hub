...
- **Never hardcode absolute paths** like `D:/Workspace/...` in `settings.local.json`. Use `${CODEX_PROJECT_DIR}` or glob patterns. Hardcoded paths are not portable across machines and break for teammates.

...
Each `/clear` is free, but the human in the loop is the bottleneck. And consolidating the work into a single long-lived session triggers Codex's auto-compact at ~70% context usage, which loses fidelity.
...
The orchestrator **never writes code**. It dispatches workers and parses their structured output. Each worker is one `Task` tool call with `subagent_type: planner | executor | verifier`. The worker runs in its own Codex session with a focused system prompt (see `.Codex/agents/<name>.md`) and exits when it has emitted the protocol tail-block.
...
The orchestrator pipes the worker's full output through `python3 .Codex/scripts/orchestrate_protocol.py parse` to extract the JSON. Everything before the tail is context-only — it never enters the orchestrator's window.
...
- Token budget exhausted → Codex crash. Resume by running `/gtr:orchestrate resume` in a fresh session.
- User Ctrl+C.
...
- **Sequential, not parallel.** `Task` calls block until the worker exits. This isn't a performance optimisation — it's a context-isolation trick. Use Codex subagents for genuinely independent work, but be aware GSD's roadmap is usually sequentially dependent.
...
```bash
python3 .Codex/scripts/orchestrate_protocol.py emit --status ok --field path=foo --field summary="hi" \
  | python3 .Codex/scripts/orchestrate_protocol.py parse
```

...
## Use AGENTS.md for architecture decisions

Document *why* decisions were made, not just what they are. Codex will apply that reasoning when making related changes.
...
Press `Shift+Tab` twice or use `/plan` to enter plan mode where Codex proposes changes before executing them. Good for refactors and migrations.

## Leverage sub-agents with the Agent tool

For tasks like "explore codebase and find all API calls", Codex can spawn a focused sub-agent that searches without polluting the main context. Codex subagents and GSD's `/gsd:execute-plan` use this pattern.

## Windows-specific notes

- Hook commands run in bash (Git Bash). Use `python` not `python3`.
- `$CODEX_PROJECT_DIR` is set to the project root automatically.

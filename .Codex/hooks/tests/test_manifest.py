...
def test_collect_files_picks_up_template_paths(tmp_path: Path):
    ...
    (tmp_path / ".Codex" / "commands" / "x.md").write_text("ok", encoding="utf-8")
    (tmp_path / "src").mkdir()
...
def test_write_manifest_is_atomic_and_leaves_no_temp(tmp_path: Path):
    ...
    leftover = list((tmp_path / ".Codex").glob(".template-manifest.*.json.tmp"))
    assert leftover == []


def test_write_manifest_overwrites_existing(tmp_path: Path):
    (tmp_path / ".Codex").mkdir()
    target = tmp_path / ".Codex" / ".template-manifest.json"

...
def test_record_block_appends_jsonl(tmp_path: Path, monkeypatch):
    ...
    log = tmp_path / ".Codex" / "hook-audit.log"
    assert log.is_file()
...
def test_record_block_swallows_oserror(tmp_path: Path, monkeypatch):
    ...
    # Make .Codex exist but mark it read-only via a file collision.
    (tmp_path / ".Codex").write_text("not-a-dir", encoding="utf-8")
    # Should not raise.
...
def _run_guard(hook_path: Path, payload: dict, project_dir: Path) -> int:
    proc = subprocess.run(
        ...
        capture_output=True,
        text=True,
        env={**os.environ, "CODEX_PROJECT_DIR": str(project_dir)},
    )

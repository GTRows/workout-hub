...
def _make_v0_1_layout(root: Path, *, with_todo_content: bool = False, with_deferred: bool = True):
    ...
    for name in v020.GTR_COMMANDS + v020.DROPPED_COMMANDS:
        (root / ".Codex" / "commands" / name).write_text(f"# {name}\n", encoding="utf-8")
    (root / ".Codex" / "VERSION").write_text("0.1.5\n", encoding="utf-8")
    ...
    (root / "AGENTS.md").write_text("Run `/setup`, then `/menu`.\n", encoding="utf-8")
...
def test_plan_idempotent_on_clean_v0_2_layout(tmp_path):
    ...
    (tmp_path / ".Codex" / "commands" / "gtr").mkdir(parents=True)
    (tmp_path / ".Codex" / "VERSION").write_text("0.2.0\n", encoding="utf-8")
...
def test_apply_renames_files_and_creates_snapshot(tmp_path):
    ...
    for name in v020.GTR_COMMANDS:
        assert (tmp_path / ".Codex" / "commands" / "gtr" / name).is_file()
        assert not (tmp_path / ".Codex" / "commands" / name).is_file()
    ...
    snapshot = tmp_path / ".Codex" / "commands.pre-v0.2.0"
    ...
    # AGENTS.md slash-command refs rewritten.
...
def test_double_apply_is_noop(tmp_path):
    ...
    assert all(a.kind != "delete" or a.src not in {f".Codex/commands/{n}" for n in v020.DROPPED_COMMANDS} for a in second)
...
def test_runner_apply_bumps_version(tmp_path):
    ...
    assert (tmp_path / ".Codex" / "VERSION").read_text(encoding="utf-8").strip() == "0.2.0"

...
Purpose: When the project has not been initialised via /gtr:setup, inject a
         reminder into the prompt's context so Codex nudges the user.
         Does NOT block — blocking creates a bootstrap deadlock when the
         `/gtr:setup` command skill hasn't loaded yet.
...
def main() -> None:
    ...
    sys.stdout.write(
        ...
        "setup wizard (no .Codex/.setup-complete marker). "
        "If the user is about to start coding work, recommend `/gtr:setup` first. "

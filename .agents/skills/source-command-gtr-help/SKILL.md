...
# source-command-gtr-help

Use this skill when the user asks to run the migrated source command `gtr-help` or `/gtr:help`.
...
**Output budget.** Follow `.Codex/docs/output-style.md`. Hard cap: 25 lines for single-topic answers. No preamble, no recap of the question, no trailing "anything else?". When in doubt, cut.

**Output language.** Read `## Communication` from `AGENTS.md` and render every human-readable phrase in that language — including the **right-hand description column** of the table-of-contents code block, the section labels above it (e.g. "TOPICS — high-level guides", "GTR COMMANDS — template lifecycle"), the INTENT cheat-sheet phrases in quotes, and every "Why / After this / Topic" body paragraph below.

Keep these tokens verbatim, regardless of language:
- Slash-command names: `/gtr:setup`, `/gsd:plan-phase`, `/commit`, etc.
- File paths and filenames: `AGENTS.md`, `IDENTITY.yaml`, `.planning/PROJECT.md`, `.Codex/.setup-complete`.
- Shell commands inside code blocks: `git push`, `python3 .Codex/scripts/manifest.py --write`.
...
- `AGENTS.md` (look for `## Communication`)
...
Also Glob `**/*.{py,ts,tsx,js,jsx,go,rs,java,kt,rb,php}` excluding `.Codex/`, `node_modules/`, `dist/`, `build/`. 10+ matches => brownfield.
...
| Step | Skip when |
|------|-----------|
| Setup                 | `.Codex/.setup-complete` exists AND `IDENTITY.yaml#identity.name` is filled (not `PROJECT_NAME`) AND `AGENTS.md` has `## Communication` |
...
GTR COMMANDS — template lifecycle
  ...
  /gtr:new-skill         Scaffold a project-specific skill under .Codex/skills/
  /gtr:new-rule          Scaffold a path-scoped rule under .Codex/rules/
  /gtr:help              This help
...
GSD COMMANDS — planning and execution
  ...
  /gsd:list-phase-assumptions  Surface Codex's assumptions before planning
...
INTENT -> COMMAND (quick map — pick the row that matches what you want to do)
  ...
  "Change the language Codex speaks to me in"        /gtr:set-language
...
1. **Setup** — `/gtr:setup` (asks language first, fills AGENTS.md and IDENTITY.yaml, checks Codex capabilities, writes `.Codex/.setup-complete`).
...
- **New project / fresh clone**: run `/gtr:setup`. It asks for conversation language *first*, then detects the stack, fills `AGENTS.md` and `IDENTITY.yaml`, checks Codex capabilities, writes `.Codex/.setup-complete`. From this point every later prompt and slash-command output speaks the language you picked.
...
- `/gsd:list-phase-assumptions <N>` — show how Codex is interpreting the phase before it commits to a plan. Catch misreadings cheaply.
...
- "Where do I start?" → `/gtr:setup` if `.Codex/.setup-complete` is missing, else `/gsd:progress`.
...
1. `/gsd:new-project` — interview about vision, constraints, success criteria. Output: `.planning/PROJECT.md` + `.planning/config.json`. In template-managed projects the brief auto-fills from `IDENTITY.yaml` / `AGENTS.md` / `README.md` and you only confirm or tweak.
...
3. `/gsd:plan-phase 1` — produces `.planning/phases/01-auth-scaffold/01-01-PLAN.md` with concrete tasks and verification gates. Optionally first run `/gsd:list-phase-assumptions 1` to see how Codex is interpreting the phase.
...
`/gtr:orchestrate` removes the manual `/clear -> /gtr:next -> /gsd:plan-phase -> /clear -> /gtr:next -> /gsd:execute-plan -> ...` loop. The architecture is documented in full at `.Codex/docs/orchestration.md`; this is the short version.
...
`status` is always present — `ok` (worker succeeded at its job), `fail` (worker hit retry budget), `blocked` (worker can't proceed without orchestrator decision). Other fields are role-specific. The orchestrator parses via `python3 .Codex/scripts/orchestrate_protocol.py parse`.
...
Guard hooks live in `.Codex/hooks/` and are registered in `.Codex/settings.json`.
...
**Customisation:** every hook has a clearly-marked `CONFIGURATION` block at the top — add new patterns there. Run the test suite afterwards: `python -m pytest .Codex/hooks/tests`.

**Exit codes:** `0` = allow, `2` = block + send stderr to Codex.

---

## Topic: manifest

`.Codex/.template-manifest.json` is a sha-256 snapshot of every template-owned file at install time. `/gtr:update` uses it to detect which files the user has modified vs which are still pristine, so upstream changes can be auto-applied to untouched files and conflict-prompted only for modified ones.

Generate or refresh: `python3 .Codex/scripts/manifest.py --write`.
Check drift: `python3 .Codex/scripts/manifest.py --check`.
...
Breaking template changes ship a per-version migration script under `.Codex/scripts/migrations/v{X}_{Y}_{Z}.py`. Each script exports `plan(root)` and `apply(root, actions)`. The runner (`.Codex/scripts/migrations.py`) discovers, orders, dry-runs, and applies them.
...
Always idempotent. Always snapshots to `.Codex/commands.pre-v{X}.{Y}.{Z}/` before reshaping. Always bumps `.Codex/VERSION` after each successful migration so partial runs are recoverable.
...
Both end with a hand-off to `/gtr:setup` for stack detection and AGENTS.md filling.
...
- **ask** — prompt the user before allowing. `git push`, `npm publish`, edits to `.Codex/`.
...
State-aware advisor. Reads the project's state (setup marker, IDENTITY, AGENTS.md, `.planning/*`, source-file count) and prints the **single next command** to run, with a short reason and the likely follow-ups.
...
First-time project wizard. Detects the stack, fills `AGENTS.md`, writes `IDENTITY.yaml`, checks Codex capabilities, hands off planning to GSD on opt-in, writes the setup marker.
...
**Output marker:** `.Codex/.setup-complete` (gitignored — per-clone, not per-repo).

---

## Command: /gtr:set-language

Set or change the conversation language for this project. Writes (or updates) the `## Communication` block in `AGENTS.md` and syncs the `language:` line in `.Codex/.setup-complete`.
...
Interactive entry point. Reads project state (`.Codex/.setup-complete`, `IDENTITY.yaml`, `.planning/`) and shows the appropriate numbered menu, asks follow-up questions up front, dispatches the right slash command.
...
- AGENTS.md placeholder text still present.
...
**Preflight:** requires `.Codex/.setup-complete`, `.planning/PROJECT.md`, `.planning/ROADMAP.md`, clean working tree (except in `resume`), and the GSD plugin installed.
...
In template-managed projects, the brief auto-fills from `IDENTITY.yaml` + `AGENTS.md` + `README.md` so you only confirm or tweak.
...
Pair with `/gsd:list-phase-assumptions` first if you want to see how Codex is interpreting the phase before committing to a plan.
...
- The conversation language for the project is set in `AGENTS.md` `## Communication`. Help output is rendered in that language — including the description column of the TOC code block. Only slash-command names, file paths, version strings, and shell commands stay verbatim.

---
name: "source-command-gtr-new-skill"
description: "[TEMPLATE] Scaffold a project-specific Codex skill under .agents/skills/."
---

# source-command-gtr-new-skill

Use this skill when the user asks to run the migrated source command `gtr-new-skill` or `/gtr:new-skill`.
...
Project-specific skills live under `.agents/skills/<slug>/`. They differ from plugin-provided skills:
...
- **Project skills** stay inside `.agents/skills/`, tracked in git. Reach for them when the capability is specific to *this* project's domain (e.g. domain glossary, internal API patterns, deployment runbook).

If the user asks for something that is general-purpose, suggest ``skill-creator` skill instead — do not bloat `.agents/skills/`.

## Steps

1. Slugify the title from `$ARGUMENTS` (lowercase, hyphenated, ASCII).
2. Refuse if `.agents/skills/<slug>/` already exists.
3. Create the directory and write `.agents/skills/<slug>/SKILL.md` using the template below.
...
5. Tell the user the skill is enabled the next time Codex reloads its skill index.
...
this is loaded into Codex's context whenever the skill triggers.>

## Anti-patterns

- <what NOT to do that would otherwise be Codex's default>
```

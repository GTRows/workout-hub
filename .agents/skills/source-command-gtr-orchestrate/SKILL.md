---
name: "source-command-gtr-orchestrate"
...
# source-command-gtr-orchestrate

Use this skill when the user asks to run the migrated source command `gtr-orchestrate` or `/gtr:orchestrate`.
...
**Output language.** Read `## Communication` from `AGENTS.md`. Render the live status stream and the milestone checkpoint prompt in that language. Slash-command names, file paths, sha values, and the protocol JSON itself stay verbatim.

**Output budget.** Per `.Codex/docs/output-style.md`. Each worker call is one log line. The milestone checkpoint is ~5 lines. The final summary is ~10 lines. No paragraph-length narration in between.
...
1. `.Codex/.setup-complete` exists. If missing, abort with: "Run `/gtr:setup` first."
...
```bash
echo "{worker-output}" | python3 .Codex/scripts/orchestrate_protocol.py parse
```
...
- Orchestration runs in the **main session**, not in a subagent. The orchestrator IS the main Codex. It must keep its own context lean (the whole point of the architecture).
- Workers know the protocol from their agent definitions in `.Codex/agents/`. You don't need to repeat the protocol contract in every spawn prompt — keep prompts short.
- For the architecture explainer, see `.Codex/docs/orchestration.md`.

---
name: "source-command-gtr-onboard"
description: "[TEMPLATE] Onboard an existing project — interactive runbook for IMPLEMENT.md (merge hooks, settings, AGENTS.md additions)."
---

# source-command-gtr-onboard

Use this skill when the user asks to run the migrated source command `gtr-onboard` or `/gtr:onboard`.
...
**Output language.** Read `## Communication` from `AGENTS.md` and conduct the interview, confirmations, and summaries in that language. File paths, code blocks, and slash-command names stay verbatim. If `## Communication` is missing, default to the language the user wrote the request in.

**Output budget.** Follow `.Codex/docs/output-style.md`. One decision per turn. No restatement of the previous decision before asking the next.
...
- `.Codex/.setup-complete` already exists. Tell the user: `This project is already onboarded. Use /gtr:update to pull the latest template; use /gtr:setup to re-run the wizard idempotently.`
...
I will:
  ...
  4. Run /gtr:setup at the end to fill AGENTS.md and IDENTITY.yaml.
...
- Tell the user one line about what changes (e.g. `Will copy .Codex/hooks/pre_guard_security.py — does not exist locally.`).
...
Copy upstream `.claude/hooks/*.py` and `.claude/hooks/optional/*.py` into local `.Codex/hooks/`, converting `.claude` paths to `.Codex`, `CLAUDE.md` to `AGENTS.md`, and hook commands to `python3`. If a hook with the same name exists locally, show the diff and ask.

### 2b. Slash commands (always safe)

Copy upstream `.claude/commands/gtr/*.md` into local `.Codex/commands/gtr/`, then regenerate the matching `.agents/skills/source-command-gtr-*/SKILL.md` wrappers. If a command with the same name exists locally, show the diff and ask.

### 2c. Scripts and tests

Copy upstream `.claude/scripts/` and `.claude/hooks/tests/` into `.Codex/`, applying the same Codex path/name conversion. Keep `plugins.py` Codex-specific; do not restore the Claude plugin CLI flow.

### 2d. Settings merge

`.Codex/settings.json` requires deep-merge:
...
### 2e. AGENTS.md additions

If the user has a AGENTS.md, do NOT overwrite. Offer each missing section individually:
...
git commit -m "chore: adopt Codex port of claude-code-template (${TEMPLATE_TAG})"
```
...
Next: run /gtr:setup to fill in AGENTS.md / IDENTITY.yaml from your project's

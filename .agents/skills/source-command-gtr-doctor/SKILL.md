---
name: "source-command-gtr-doctor"
description: "[TEMPLATE] Health check the template state: setup marker, plugins, AGENTS.md placeholders, planning state (GSD), hooks, secret leakage."
---

# source-command-gtr-doctor

Use this skill when the user asks to run the migrated source command `gtr-doctor` or `/gtr:doctor`.
...
**Output budget.** Follow `.Codex/docs/output-style.md`. One short line per check. Skip checks that pass with no notes — only surface findings worth acting on. Cap the whole report at ~30 lines unless drift is severe.

**Output language.** Read `## Communication` from `AGENTS.md` and render all explanations, summaries, and recommendations in that language. Section headings, file paths, code blocks, and tool output stay verbatim. If `## Communication` is missing, default to the language the user wrote the request in.

---

## 1. Setup marker

- Does `.Codex/.setup-complete` exist?
- If yes → parse and print `date`, `stack`, `type`.
- If no → state: `Not set up. Run /gtr:setup.`

---

## 2. `AGENTS.md` placeholders
...
It prints the Codex capabilities this template expects. Compare that list with the active Plugins/Skills list in the current Codex session. Report missing capabilities as informational unless the user is trying to use that feature now.

For this project, the key practical checks are:

- Browser plugin available for local UI verification.
- GitHub plugin available for PR/CI work.
- Repo-local command skills under `.agents/skills/source-command-*` are present.
- Repo-local `frontend-design` skill is present for UI redesign work.

## 6. Hooks

Read `.Codex/settings.json`. Confirm:
...
- If `.Codex/VERSION` exists, print its content.
...
- If `.Codex/.template-manifest.json` exists, run:
  ```bash
...
- If the manifest is missing, suggest running `/gtr:update` (which will regenerate it) or `python3 .Codex/scripts/manifest.py --write` to seed it.
...
- **Manifest drift volume**: if `python3 .Codex/scripts/manifest.py --check` reports more than 5 modified files, flag: "Significant manifest drift — `/gtr:update` will require many conflict decisions."
- **Migration backlog**: run `python3 .Codex/scripts/migrations.py --check` against the upstream version (already known from section 8). If migrations are pending, flag: "Schema migrations pending."

Each flag is one short line. Do not score or rank — let the user decide.

## 11. Token usage summary (last 7 days)

If `.Codex/usage-log.jsonl` exists, read the last 7 days of records and print:
...
If `.Codex/hook-audit.log` exists, read the last 7 days of records and print:

...
Before doing any coding work, check for `.Codex/.setup-complete`.
...
- `/gtr:set-language [lang]` — set or change the conversation language (writes `## Communication` in `AGENTS.md`).
...
- `/gtr:orchestrate [scope]` — run phases end-to-end via planner/executor/verifier subagents. Default scope: roadmap-wide with milestone-end checkpoints. See `.Codex/docs/orchestration.md`.
- `/gtr:release <version>` — prepare a release (bump, rotate CHANGELOG, commit, tag). Never pushes.
- `/gtr:new-adr <title>` — scaffold an Architecture Decision Record under `docs/adr/`.
- `/gtr:new-migration <title>` — scaffold a database migration using the detected migration tool.
- `/gtr:new-rule <title>` — scaffold a path-scoped Codex rule.
- `/gtr:new-skill <title>` — scaffold a project-specific Codex skill.
- Codex app/plugin commands depend on the active session. Prefer repo-local `source-command-gtr-*` skills for the commands above.
...
| `export/` | `/api/export/Codex-summary` and import endpoint | PHASE 5 |
...
Keep the `PROTECTED_EXACT` set in `.Codex/hooks/pre_guard_release_files.py` in sync with this list. Do not edit the hook without user confirmation.
...
- Commits are authored by the user via local git config. Do NOT add `Co-Authored-By: Codex` trailers to commit messages.
